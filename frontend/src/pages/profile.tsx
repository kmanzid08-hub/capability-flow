...
function isSupportedEvidenceDate(value: unknown): boolean {
...
function suggestionValidationHint(suggestion: ProfileSuggestion): string | null {
  ...
  if (suggestion.category === "employment" || suggestion.category === "project") {
    ...
    if (!isSupportedEvidenceDate(startDate) || startDate === null || startDate === undefined || startDate === "") {
      return "A start date is required before this record can be saved. Use the exact YYYY, YYYY-MM, or YYYY-MM-DD precision stated in the supporting evidence; do not guess missing precision.";
    }
    if (!isSupportedEvidenceDate(payload.end_date)) {
      return "The end date is incomplete or invalid. Use YYYY, YYYY-MM, or YYYY-MM-DD from the supporting evidence, or leave it blank if the document does not state an end date.";
    }
  }

  if (suggestion.category === "certification") {
    if (!isSupportedEvidenceDate(payload.issue_date) || !isSupportedEvidenceDate(payload.expiry_date)) {
      return "A certification date is incomplete or invalid. Use YYYY, YYYY-MM, or YYYY-MM-DD exactly as stated in the evidence, or leave the optional date blank when it is not stated.";
    }
  }
