...
@router.get(
    "/ai-suggestions",
    response_model=list[ProfileSuggestionResponse],
)
async def list_suggestions(
    ...
    user: CurrentUser,
    session: SessionDep,
    suggestion_status: str | None = Query(
        default=None,
        alias="status",
    ),
...
async def accept_all_suggestions(
    ...
    return await _service(session, membership, user).accept_all(person_id)
